...
struct PCMTutorial {
  ...
  Configurable<float> maxTPCNsigmaEl{"maxTPCNsigmaEl", 4.0,
                                     "max. TPC n sigma for electron"};
  ...
  HistogramRegistry fRegistry{
      ...
      {
          {"Event/hVertexZ",
           "z vtx; z vtx (cm);Number of Events",
           {HistType::kTH1F, {{100, -50.f, +50.f}}}},
          {"V0/hPt",
           "p_{T} of V0;V0 p_{T} (GeV/c)",
           {HistType::kTH1F, {{1000, 0.f, 10.f}}}},
          {"V0/hMGamma",
           "mass #gamma;m_{ee} (GeV/c^{2})",
           {HistType::kTH1F, {{100, 0.f, 0.1f}}}},
          {"V0/hEtaPhi",
           "#eta vs. #varphi of V0;#varphi (rad.);#eta",
           {HistType::kTH2F, {{72, 0.f, TMath::TwoPi()}, {200, -1.f, +1.f}}}},
          {"V0/hAP",
           "Armenteros Podolanski;#alpha;q_{T} (GeV/c)",
           {HistType::kTH2F, {{200, -1, +1}, {250, 0.0, 0.25f}}}},
          {"V0Leg/hdEdx_Pin",
           "dE/dx in TPC;p_{in} (GeV/c);TPC dE/dx",
           {HistType::kTH2F, {{1000, 0.f, 10.f}, {200, 0, 200}}}},
          {"Diphoton/hMgg",
           "2-photon invariant mass;m_{#gamma#gamma} (GeV/c^{2})",
           {HistType::kTH1F, {{200, 0.f, 0.8f}}}},
      },
  };

  void init(InitContext const &) {
    auto hEventCounter = fRegistry.add<TH1>("hEventCounter", "hEventCounter",
                                            kTH1F, {{5, 0.5f, 5.5f}});
  ...
  template <typename TTrack, typename TV0> bool checkV0(TV0 const &v0) {
    // if (!checkAP(v0.alpha(), v0.qtarm(), 0.95, 0.03)) { // select photon
    // conversions
    //   return false;
    // }
  ...
  template <typename TTrack> bool checkV0leg(TTrack const &track) {
  ...
  using MyTracks = soa::Join<aod::TracksIU, aod::TracksCovIU, aod::TracksExtra,
                             aod::TracksDCA, aod::pidTPCFullEl>;
  ...
  void process(MyCollisions const &collisions, aod::BCsWithTimestamps const &,
               aod::V0Datas const &v0s, MyTracks const &tracks) {
    ...
    for (auto &collision : collisions) {
      ...
      for (auto &v0 : v0s_per_coll) {
        ...
        // retrieve information about electrons
        ...
        for (auto &leg : {pos, ele}) {
          fRegistry.fill(HIST("V0Leg/hdEdx_Pin"), leg.tpcInnerParam(),
                         leg.tpcSignal());
        } // end of leg loop
      }   // end of v0 loop

      // Combine V0s and calculate pi0 candidates
      for (auto &[g1, g2] : combinations(CombinationsStrictlyUpperIndexPolicy(
               v0s_per_coll, v0s_per_coll))) {
...
WorkflowSpec defineDataProcessing(ConfigContext const &cfgc) {
  return WorkflowSpec{
      adaptAnalysisTask<PCMTutorial>(cfgc, TaskName{"pcm-tutorial"})};
}
