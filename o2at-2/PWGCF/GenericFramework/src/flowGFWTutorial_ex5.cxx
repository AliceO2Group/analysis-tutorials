...
struct GfwTutorial {
  ...
  using aodTracks = soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection, aod::TracksExtra, aod::pidBayes, aod::pidTPCFullPi, aod::pidTPCFullKa, aod::pidTPCFullPr,aod::pidTOFFullPi, aod::pidTOFFullKa, aod::pidTOFFullPr>>;
  ...
  void init(InitContext const&)
  {
    ...
    registry.add("c22_pi", "", {HistType::kTProfile,{axisMultiplicity}});
    registry.add("c22_ka", "", {HistType::kTProfile,{axisMultiplicity}});
    registry.add("c22_pr", "", {HistType::kTProfile,{axisMultiplicity}});

    fGFW->AddRegion("full", -0.8, 0.8, 1, 1);
    fGFW->AddRegion("fullPi",-0.8,0.8,1,2);
    fGFW->AddRegion("fullKa",-0.8,0.8,1,4);
    fGFW->AddRegion("fullPr",-0.8,0.8,1,8);

    corrconfigs.push_back(fGFW->GetCorrelatorConfig("full {2 -2}", "ChFull22", kFALSE));  
    corrconfigs.push_back(fGFW->GetCorrelatorConfig("fullPi {2 -2}", "PiFull22", kFALSE));
    corrconfigs.push_back(fGFW->GetCorrelatorConfig("fullKa {2 -2}", "KaFull22", kFALSE));
    corrconfigs.push_back(fGFW->GetCorrelatorConfig("fullPr {2 -2}", "PrFull22", kFALSE));
    fGFW->CreateRegions();
  }
...
int GetBayesPIDIndex(TTrack track) {
  ...
  void process(aodCollisions::iterator const& collision, aod::BCsWithTimestamps const&, aodTracks const& tracks)
  {
    ...
    for (auto& track : tracks) {
      ...
      if(pidIndex) fGFW->Fill(track.eta(), 1, track.phi(), wacc * weff, 1<<(pidIndex));
      fGFW->Fill(track.eta(), 1, track.phi(), wacc * weff, 1);
    }

    //Filling c22 with ROOT TProfile
    FillProfile(corrconfigs.at(0), HIST("c22"), cent);
    FillProfile(corrconfigs.at(1), HIST("c22_pi"), cent);
    FillProfile(corrconfigs.at(2), HIST("c22_ka"), cent);
    FillProfile(corrconfigs.at(3), HIST("c22_pr"), cent);
  }
};
